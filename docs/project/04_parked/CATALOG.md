...
- **Status: partially un-parked, 2026-09-15; color editing also un-parked, 2026-09-17; multiline-aware highlighting also un-parked, 2026-09-17.** [BF-027](../BETA_FEEDBACK.md#bf-027--file-type-syntax-highlighting-profiles-xml-yml-json-sfm-md-tsv-csv-)/[BF-041](../BETA_FEEDBACK.md#bf-041-p3--syntax-highlighting-extension-point-does-not-exist) implemented a deliberately bounded first slice: a profile registry (Plain Text/Markdown/XML/JSON/YAML/SFM/CSV/TSV), extension-to-profile mapping with a user override setting, and presentation-only highlighting in the editor.
  - ~~Not incremental or multiline-aware.~~ Resolved by [BF-071](../BETA_FEEDBACK.md#bf-071--multiline-aware-syntax-highlighting): `SyntaxProfile.tokenize` now threads an opaque per-profile state value from one logical line's end to the next line's start, so a multi-line XML `<!-- -->` comment/`<![CDATA[ ]]>` section, a Markdown fenced ` ``` `/`~~~` code block, or a YAML `|`/`>` block scalar highlights correctly across the lines it spans. Stays bounded and huge-file-safe by construction — the UI layer caches only one small state value per logical line (never a parse tree or the whole document), built up incrementally as lines are painted in visible order, falling back to the profile's neutral initial state rather than walking the whole document backward when a scroll jump lands somewhere never yet visited. **Remaining, deliberately not done:** state threads across whole logical lines only, not within one very long logical line's own wrapped/horizontal-scroll rows; CSV's quoted-field embedded newlines are unhandled (they'd change what a "row" even is, entangling this with the document's own line segmentation rather than staying presentation-only); Markdown HTML block comments are unhandled (no single-line recognition existed for them before this pass either, so adding them is new tokenizing scope, not an extension of a flagged gap). See the [BF-071](../BETA_FEEDBACK.md#bf-071--multiline-aware-syntax-highlighting) entry for the full implementation record.
...
- **Remaining parked scope:** true incremental parsing/language-grammar support beyond the bounded regex-based profiles above (e.g. actual per-language grammars, LSP-grade tokenization) — a materially larger, separately-scoped effort.
