...
The feedback work began on isolated branch `work/b2-feedback` at historical checkpoint `5724f6c`. The initial ancestry observation (`origin/main` at `ca75966`, local `main` seven ahead) and the six already-present feedback commits remain historical context; none needs reapplication. Complete reviewed feedback code candidate `3e20214` contains the integrated BF-001–BF-010 series. On 2026-09-08, local `main`, `origin/main`, and the work branch synchronized at documentation checkpoint `b152f0e`, which contains that verified source plus documentation-only follow-ups.

The three metadata authorities agree on A22: `VERSION` is `v0.001a22`, while `pyproject.toml` and `src/uniti/__init__.py` are `0.1a22`. Historical source verification on `5724f6c` passed 1,476 tests with six platform-policy skips in 93.01 seconds, deep self-check 21/21, and native Cocoa combined smoke including explicit Quit and session restore. Latest hosted-green checkpoint `aab3f3c` passed all four lanes in run `34253008439`, including full suite, deep self-check, native/offscreen smoke, and sustained checks. Later runs `34253913469`, `34253932056`, and synchronized-checkpoint run `34256598416` did not execute because GitHub reported an account billing/payment or spending-limit block; no code result exists and no gate was weakened.

Reviewed Tasks 2, 4, 5, 6, 7, and 8 are in the candidate: first-use progress and the durability-warning fix; centered U+0020 markers plus persisted hotkey access; 80% gutter typography and truthful current/on-save EOL behavior with all-view refresh; complete Find/Replace Lucide controls and aligned `\\N :` capture rows; named Paper/Slate plus custom theme editing backed by one bounded atomic profile file; and application-bundled Noto fallback with bounded shaped LTR editing. The detailed behavior and limits are in the [LTR text layout contract](../../../docs/ltr-text-layout.md).

Candidate `3e20214` passed 1,646 tests with six expected platform skips in 95.63 seconds, compilation, diff checks, 21/21 deep self-check, 108 native Cocoa targeted tests, and native Cocoa/offscreen combined smoke including Quit/session restore. Independent final source rereview is clean. The rebuilt isolated wheel contains 40 exact resources—13 fonts, 11 SVGs, two themes, and 14 manifest/notice files—and renders mixed text and Find/Replace from the installed wheel. Uncontended scroll, typing, and giant-line gates passed with unchanged thresholds; the 72-action mixed-script scenario also passed. All four local hosted-profile sustained families passed five measured cycles plus warmup with no contention.
...
- Complete reviewed feedback code candidate: `3e20214`
- Synchronized documentation checkpoint: `b152f0e` on local `main`, `origin/main`, and `work/b2-feedback`; source remains `3e20214`
- Latest hosted-green feedback checkpoint: `aab3f3c`, run `34253008439` (all four lanes)
- Hosted constraint: later requested jobs did not execute because of the reported GitHub account billing/payment or spending-limit block
...
Physical Chinese/Korean IME composition/commit/cancel on Windows, macOS and Linux remains qualification work; synthetic events do not close it. Separately distribute the BF-002 correction to the affected Windows and Mac hosts and require explicit Quit/relaunch confirmation before closing the blocker. Restore same-integrated-commit hosted qualification only after GitHub permits jobs to execute. A22 remains active: seven distinct real-use days, A23 executable/health delivery, A24 qualification, and B1 private executable human feedback remain required before B2 identity promotion. Do not infer those gates from source integration or hosted automation, and do not tag, release, or contact testers from this handover alone.
