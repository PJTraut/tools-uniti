...
- Complete reviewed feedback source: BF-012–BF-068, plus BF-040/042 (the last two escalated architecture findings from the P1 review), BF-064's multi-checkpoint RTL horizontal-scroll fix, BF-063's syntax color editing (ADR-0011), and BF-030/031's benchmark coverage for many open documents and large history at scale, committed and integrated on `main` at `3861e63` (previously `ca05d0b`, then `6d0e90f`/`d511723`/`0d75094`/`05483ec`/`81c59e9`/`706a183`/`ab67e87`/`6625cad`; see the [B3 milestone plan](../02_plans/v0.001b3-find-replace-and-editor-refinement-beta.md) for the full item table).
- `origin/main` was brought up to `6625cad` this capture; local `main` is ahead of it by one commit (`3861e63`), not yet pushed.
...
- Per-document task-pool fairness (BF-040: same-priority tasks now round-robin across documents instead of pure FIFO) and an open-ended, forward-compatible session-schema `extra` blob on `ViewRecord`/`DocumentRecord`/`FindReplaceManifestRecord` (BF-042, `SESSION_SCHEMA` now 6) are integrated — the last two escalated architecture findings from the P1 review. BF-030/031 (multi-document and large-scale benchmark coverage), unblocked by BF-040, are now implemented as acceptance tests exercising the real application and history/session APIs directly at the real 512 MiB cache, 32 MiB/50-transaction, and 256 MiB aggregate limits — see the [Beta Feedback Log](../BETA_FEEDBACK.md) for each.
...
Local full suite at `3861e63`: **1,981 passed, 6 expected platform skips, no failures**. This supersedes the `ab67e87` count of 1,973 (the 8-test difference is BF-030/031's new document-scale, history-scale, and aggregate-admission tests); the 8 local bootstrap/metadata-mismatch failures previously recorded at `0d75094` continue not to reproduce, as they did not at `81c59e9`/`ab67e87` either — see that capture's note and the [Pool 1 plan](../03_implemented/milestones/2026-09-17-pool-1-task-fairness-and-schema-flexibility-plan.md) for the original characterization. This supersedes the `ca05d0b` count below, which is retained for history: **1,830 passed, 6 expected platform skips**, including the regressions added for that batch (BF-017's single- and two-window shortcut-delivery tests, BF-056's match-report-sizing test) and the rewritten menu-structure test for BF-057. One order-dependent flaky failure (`test_palette_change_rebuilds_group_formats_with_accessible_contrast`, in `tests/ui/test_find_replace_contract.py`) was confirmed present on the pre-session code too, when the full file runs in a particular order; it is unrelated to that batch's changes and passes in isolation.

No fresh hosted CI run or native-host confirmation exists for this B3 candidate. The historical B2 verification below remains as it was captured; it is evidence for `3e20214`/`596c88f`, not for the current `3861e63` tip.
...
1. Push `3861e63` to `origin/main` when ready (local is one commit ahead as of this capture).
...
3. BF-017 and BF-025 received native confirmation 2026-09-18 and are resolved. BF-019's indentation/truncation fix was also confirmed 2026-09-18, but that confirmation surfaced a new, not-yet-fixed finding (the `\N :` capture-group label renders ~10% smaller than the matched-text content) — see [BF-019](../BETA_FEEDBACK.md#bf-019--match-report-capture-group-row-indentation-and-truncated-label). Continue obtaining native confirmation for any other item still recorded as pending native/affected-host evidence.
