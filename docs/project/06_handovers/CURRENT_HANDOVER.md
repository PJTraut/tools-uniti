...
Decision-dependent items awaiting a scope call before implementation: BF-063's syntax-category-color editor (the extension→profile-mapping half is implemented; the color half needs the same kind of un-parking decision [ADR-0009](../05_decisions/ADR-0009-bounded-open-folder-by-type.md) recorded for BF-029). BF-029/BF-044 and BF-040/042 are resolved — see the [Beta Feedback Log](../BETA_FEEDBACK.md) for each.
