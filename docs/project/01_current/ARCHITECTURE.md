...
Date: 2026-09-08
Baseline: A22 identity retained; reviewed feedback source integrated and four-lane hosted-green through `aab3f3c`
...
`UNITITextView` continues to paint only visible document content through the custom virtual viewport. The current completed font policy resolves a concrete platform-preferred or installed fixed-pitch font and verifies representative Latin and Cyrillic coverage; bundled Noto fallback and wider LTR layout work remain in progress. One shortcut policy resolves standard actions through Qt native keys, admits only valid portable persisted overrides, and reports bounded unknown/invalid/conflicting entries. The view applies clamped 50–300% font scaling and handles primary-modifier wheel zoom without transferring text ownership to Qt. Its gutter derives a separate font at 80% of the document point size, retains the document baseline and row height, and settles progressive digit-width changes before wrapped-row paint and hit testing. Global whitespace modes are Off, EOL, Spaces & Tabs, Invisible Unicode, and All. Markers are transient overlays over committed text, reuse each visible row's `QTextLayout` geometry, classify logical LF/CRLF/CR through at most a two-character terminator read, and admit at most 4,096 draw operations per frame; overflow is represented by one `+N` marker. Compact special-space and zero-width marks are drawn by `whitespace_painter`; ordinary U+0020 uses one antialiased dot centered from its real layout bounds, tabs use `»`, and line endings use `␊`/`␍`/`␍␊`. Marker positions translate document code-point offsets into Qt UTF-16 offsets. An application event filter tracks the configurable modifier-only inspection hold without consuming commands. While held, each viewport can show a bounded key of its visible marker types and a Unicode name/code-point readout for one selected code point in the focused view. Release or application deactivation clears inspection. Text, offsets, selections, clipboard contents, IME preedit, search, history, recovery, and saved bytes are unchanged.
...
The pinned read-only workflow defines exactly macOS 15/Python 3.12, Windows 2025/Python 3.12, Ubuntu 24.04/Python 3.12, and Ubuntu 24.04/newest stable Python lanes. Each begins with the public launcher and a clean owned runtime, then runs complete pytest, compile, deep self-check, offscreen smoke, native smoke, sustained checks, and skip verification. Sanitized evidence uploads only on failure. Hosted run `34253008439` passed all four lanes on current feedback checkpoint `aab3f3c`. A separate native Cocoa theme regression found a System-preview palette leak; reviewed correction `e045b0f` is awaiting integration.

## Planned-change boundary

The complete a20 Recovery & Session Alpha, A21 Editor Layout and Visibility workstream, A21 Cross-Platform Alpha, A22 source work, and reviewed BF-001–BF-005/BF-007–BF-010 changes above are implemented architecture. The identity remains A22 because its real-use gate and the A23, A24, and B1 predecessors remain open. BF-006 Noto fallback/layout work is still in progress; named-script support and native IME qualification are not current architecture. Multi-code-point grapheme inspection, RTL/mixed-direction editing, extension-sensed file-type profiles, and syntax highlighting remain outside current scope.
