...
`UNITITextView` continues to paint only visible document content through the custom virtual viewport. One Qt font policy resolves a concrete platform-preferred or installed fixed-pitch font and verifies representative Latin and Cyrillic coverage before the editor uses it. One shortcut policy resolves standard actions through Qt native keys, admits only valid portable persisted overrides, and reports bounded unknown/invalid/conflicting entries. The view applies clamped 50–300% font scaling and handles primary-modifier wheel zoom without transferring text ownership to Qt. Global whitespace modes are Off, EOL, Spaces & Tabs, Invisible Unicode, and All. Markers are transient overlays over committed text, reuse each visible row's `QTextLayout` geometry, classify logical LF/CRLF/CR through at most a two-character terminator read, and admit at most 4,096 draw operations per frame; overflow is represented by one `+N` marker. Compact special-space and zero-width marks are drawn by `whitespace_painter`; tabs use `»` and line endings use `␊`/`␍`/`␍␊`. Marker positions translate document code-point offsets into Qt UTF-16 offsets. An application event filter tracks the configurable modifier-only inspection hold without consuming commands. While held, each viewport can show a bounded key of its visible marker types and a Unicode name/code-point readout for one selected code point in the focused view. Release or application deactivation clears inspection. Text, offsets, selections, clipboard contents, IME preedit, search, history, recovery, and saved bytes are unchanged.
...
`ui.icons` provides palette-aware `QIcon` rendering for a pinned nine-icon Lucide SVG subset packaged with its upstream ISC/MIT notices. Find/Replace uses these icons for actions, field clear controls, cancellation, and report visibility. Icons render at the requested device pixel ratio and are tinted from the current palette, preserving native disabled-state opacity. No icon font, runtime download, or additional Python dependency is required.
